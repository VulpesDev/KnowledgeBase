# DataBase
Interesting links I keep finding when I'm bored

# The Clean Code Movement
## Design Patterns

[Design Patterns (THE GOAT)](https://refactoring.guru/design-patterns)

[Design Patterns](https://en.wikipedia.org/wiki/Design_Patterns)

[Singleton](https://en.wikipedia.org/wiki/Singleton_pattern)

[Dependency injection](https://en.wikipedia.org/wiki/Dependency_injection)

[Factory Patterns](https://medium.com/@fairushyn/a-deep-dive-into-factory-patterns-in-c-net-a4b0bf1996b5)

[Decorator Pattern in C# - 3 versions](https://www.csharp.com/article/decorator-pattern-in-c-sharp-3-versions/)

[Facade Method Design Pattern - GeeksforGeeks](https://www.geeksforgeeks.org/facade-design-pattern-introduction/)

[Flyweight Design Pattern - GeeksforGeeks](https://www.geeksforgeeks.org/flyweight-design-pattern/)

### Principles

[SOLID](https://en.wikipedia.org/wiki/SOLID)

[KISS](https://en.wikipedia.org/wiki/KISS_principle)

[Liskov substitution](https://en.wikipedia.org/wiki/Liskov_substitution_principle)

[You ain't gonna need it](https://en.wikipedia.org/wiki/You_aren%27t_gonna_need_it)

[Shut up already](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself)

# Useful Binaries

[Magnificent app which corrects your previous console command.](https://github.com/nvbn/thefuck)

[yorukot/superfile: Pretty fancy and modern terminal file manager](https://github.com/yorukot/superfile?tab=readme-ov-file)

# Interesting Contraptions

[Fast-forward long pauses between sentences](https://github.com/WofWca/jumpcutter)

[FujiwaraChoki/project-gen: A Tool to help beginners with their README structure.](https://github.com/FujiwaraChoki/project-gen)

[karaketir16/file2video: You can convert any file to video. Than you can upload them to youtube.](https://github.com/karaketir16/file2video)

[fast4x/RiMusic: A multilingual Android application for streaming music from YouTube Music.](https://github.com/fast4x/RiMusic?tab=readme-ov-file)

[Social Media APIs for Posting, Scheduling, and Analytics | Ayrshare](https://www.ayrshare.com/)

[VikParuchuri/marker: Convert PDF to markdown + JSON quickly with high accuracy](https://github.com/VikParuchuri/marker)

[CorentinTh/it-tools: Collection of handy online tools for developers, with great UX.](https://github.com/CorentinTh/it-tools)

[WerWolv/ImHex: 🔍 A Hex Editor for Reverse Engineers, Programmers and people who value their retinas when working at 3 AM.](https://github.com/WerWolv/ImHex?tab=readme-ov-file)

[dolphin-emu/dolphin: Dolphin is a GameCube / Wii emulator, allowing you to play games for these two platforms on PC with improvements.](https://github.com/dolphin-emu/dolphin)

[deepseek-ai/Janus: Janus-Series: Unified Multimodal Understanding and Generation Models](https://github.com/deepseek-ai/Janus)

[epezent/implot: Immediate Mode Plotting](https://github.com/epezent/implot?tab=readme-ov-file)

[espeak-ng/espeak-ng: eSpeak NG is an open source speech synthesizer that supports more than hundred languages and accents.](https://github.com/espeak-ng/espeak-ng)

[excalidraw/excalidraw: Virtual whiteboard for sketching hand-drawn like diagrams](https://github.com/excalidraw/excalidraw)

[paperless-ngx/paperless-ngx: A community-supported supercharged version of paperless: scan, index and archive all your physical documents](https://github.com/paperless-ngx/paperless-ngx)

[blakeblackshear/frigate: NVR with realtime local object detection for IP cameras](https://github.com/blakeblackshear/frigate?tab=readme-ov-file)

[is-a-dev/register: Grab your own sweet-looking '.is-a.dev' subdomain.](https://github.com/is-a-dev/register)

[penpot/penpot: Penpot: The open-source design tool for design and code collaboration](https://github.com/penpot/penpot)

[LobeChat Knowledge Base Launch — Fro... · LobeHub](https://lobehub.com/blog/knowledge-base)

[n4ze3m/page-assist: Use your locally running AI models to assist you in your web browsing](https://github.com/n4ze3m/page-assist)

[nlohmann/json: JSON for Modern C++](https://github.com/nlohmann/json)

[practical-tutorials/project-based-learning: Curated list of project-based tutorials](https://github.com/practical-tutorials/project-based-learning)

[deskflow/deskflow: Deskflow lets you share one mouse and keyboard between multiple computers on Windows, macOS and Linux. It's like a software KVM (but without video).](https://github.com/deskflow/deskflow)

[Deploying Hummingbot on a Raspberry Pi | by hummingbot | Hummingbot Blog | Medium](https://medium.com/hummingbot/deploying-hummingbot-on-a-raspberry-pi-32120a0314a)

[yt-dlp/yt-dlp: A feature-rich command-line audio/video downloader](https://github.com/yt-dlp/yt-dlp)

[nautechsystems/nautilus\_trader: A high-performance algorithmic trading platform and event-driven backtester](https://github.com/nautechsystems/nautilus_trader)


# Networking

[Onion routing - Wikipedia](https://en.wikipedia.org/wiki/Onion_routing#History)

[Garlic routing - Wikipedia](https://en.wikipedia.org/wiki/Garlic_routing)

[I2P - Wikipedia](https://en.wikipedia.org/wiki/I2P)

[Ricochet Refresh](https://www.ricochetrefresh.net/)

[Perfect Dark (P2P) - Wikipedia](https://en.wikipedia.org/wiki/Perfect_Dark_(P2P))

[GyulyVGC/sniffnet: Comfortably monitor your Internet traffic 🕵️‍♂️](https://github.com/GyulyVGC/sniffnet)

# Security

[CTF Collection](https://github.com/collections/ctf-cybersec-resources)

# Interesting Challenges

[jamesroutley/write-a-hash-table: ✏️ Learn how to write a hash table in C](https://github.com/jamesroutley/write-a-hash-table)

[README | Programming The Blockchain in C#](https://programmingblockchain.gitbook.io/programmingblockchain)

[Home · ssloy/tinyraytracer Wiki](https://github.com/ssloy/tinyraytracer/wiki)

[os01 | Bootstrap yourself to write an OS from scratch. A book for self-learner.](https://tuhdo.github.io/os01/)

# Banking

[The Complete Guide to Payments Tokenization](https://www.aciworldwide.com/blog/a-primer-on-tokens-tokenization-payment-tokens-and-merchant-tokens)

# Math

[ossu/math: 🧮 Path to a free self-taught education in Mathematics! (THE GOAT)](https://github.com/ossu/math)

[Arithmetica - Wikipedia](https://en.wikipedia.org/wiki/Arithmetica)

[Euclidean geometry - Wikipedia](https://en.wikipedia.org/wiki/Euclidean_geometry)

[Euclid's Elements - Wikipedia](https://en.wikipedia.org/wiki/Euclid%27s_Elements)

[Euclid's theorem - Wikipedia](https://en.wikipedia.org/wiki/Euclid%27s_theorem)

[Banach–Tarski paradox - Wikipedia](https://en.wikipedia.org/wiki/Banach%E2%80%93Tarski_paradox)

[Introduction to Mathematical Thinking - Discussions | Coursera](https://www.coursera.org/learn/mathematical-thinking/discussions/forums/5Dv4WCj2Eea8jw6UvTi2Tw/threads/N9nUadauEe6muwr_9fW95w)

[De Morgan's laws - Wikipedia](https://en.wikipedia.org/wiki/De_Morgan%27s_laws)

# Electronics

[(PDF) SceNDD: A Scenario-based Naturalistic Driving Dataset](https://www.researchgate.net/publication/365103967_SceNDD_A_Scenario-based_Naturalistic_Driving_Dataset)

[TixiaoShan/LIO-SAM: LIO-SAM: Tightly-coupled Lidar Inertial Odometry via Smoothing and Mapping](https://github.com/TixiaoShan/LIO-SAM?tab=readme-ov-file)

[livox horizon lidar 360 - Search](https://www.bing.com/search?q=livox%20horizon%20lidar%20360&qs=n&form=QBRE&sp=-1&ghc=1&lq=0&pq=livox%20horizon%20lidar%20360&sc=12-23&sk=&cvid=45DF538B12C54410A04F2748A527C070&ghsh=0&ghacc=0&ghpl=&ntref=1)

[Livox Mid-360](https://www.livoxtech.com/mid-360)

# Collections

[poteto/hiring-without-whiteboards: ⭐️ Companies that don't have a broken hiring process](https://github.com/poteto/hiring-without-whiteboards)

[hagezi/dns-blocklists: DNS-Blocklists: For a better internet - keep the internet clean!](https://github.com/hagezi/dns-blocklists)


# Misc

[NASA claims China can slow Earth rotation and make days longer](https://www.msn.com/en-xl/news/other/nasa-claims-china-can-slow-earth-rotation-and-make-days-longer/ar-AA1wUtjJ?ocid=msedgntp&pc=U531&cvid=381ab7855dc149eebafef5771bf049fa&ei=24)

[Want to move to Spain in 2025? Beware of new property tax, anti-tourism protests and Airbnb bans](https://www.msn.com/en-xl/news/other/want-to-move-to-spain-in-2025-beware-of-new-property-tax-anti-tourism-protests-and-airbnb-bans/ar-AA1xjL9X?ocid=msedgntp&pc=U531&cvid=8fa7f61210f2484a90e0e2daabc23157&ei=58)

[Maps reveal where in the brain different types of LOVE are felt](https://www.msn.com/en-xl/news/other/maps-reveal-where-in-the-brain-different-types-of-love-are-felt/ar-AA1pwDTp?ocid=msedgdhp&pc=U531&cvid=d02cf8b2b0014ca98c23e595e7200958&ei=28#)

[Live Webcam Newport City, Vermont](https://webcamera24.com/camera/usa/newport-city-cam/)

[What's the answer to this riddle? | Learn English](https://www.ecenglish.com/learnenglish/lessons/whats-answer-riddle)
